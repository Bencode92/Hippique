...
const rankingLoader = {
    ...
    // NOUVEAU: Poids pour la position à la corde 
    CORDE_WEIGHTS: { 
        "interieur": { adjustment: 0.02 }, // Bonus pour les numéros de corde favorables (corde 1, 2, 3) 
        "moyenne": { adjustment: 0.00 },   // Neutre pour positions moyennes 
        "exterieur": { adjustment: -0.02 } // Malus pour les grands numéros (cordes extérieures) 
    },
    
    // NOUVEAU: Multiplicateurs d'impact de la corde selon la distance
    CORDE_DISTANCE_MULTIPLIERS: {
        "sprint": 1.2,    // Impact plus important sur les sprints
        "mile": 1.0,      // Impact standard sur mile
        "middle": 0.7,    // Impact réduit sur moyenne distance
        "staying": 0.4,   // Impact minimal sur longue distance
    },
    ...
    // NOUVEAU: Extraction du numéro de corde à partir du format "(Corde:XX)"
    extractCordeNumber: function(cordeStr) {
        if (!cordeStr) return null;
        
        // Format "(Corde:XX)" ou simplement "XX"
        const match = String(cordeStr).match(/(?:\(Corde:)?(\d+)(?:\))?/i);
        if (match && match[1]) {
    ...
    // NOUVEAU: Bucket de corde (interieur/moyenne/exterieur)
    getCordeBucket: function(corde, totalParticipants) {
        if (!corde || corde <= 0 || !totalParticipants) return "moyenne";
        
        const ratio = corde / totalParticipants;
        
        if (ratio <= 0.33) return "interieur";    // Corde basse = avantage
        if (ratio >= 0.66) return "exterieur";    // Corde haute = désavantage
        return "moyenne";
    },
    ...
    getWeights: function(course) {
        ...
        const defaultWeights = { 
            cheval: 0.480, jockey: 0.131, entraineur: 0.105, eleveur: 0.087, proprietaire: 0.070, poids_porte: 0.097, corde: 0.03
        };
        ...
        // Distance (35%) + Taille (26%) + Position (17%) + Type (9%) + Poids porté (10%) + Corde (3%)
        const result = {};
        const keys = ['cheval', 'jockey', 'entraineur', 'eleveur', 'proprietaire'];
        
        keys.forEach(k => {
            // Calculer la moyenne pondérée avec les nouveaux coefficients
            result[k] = (dw[k] * 0.35) + (sw[k] * 0.26) + (pw[k] * 0.17) + (tw[k] * 0.09);
            // Réduire de 13% pour faire place au poids porté (10%) et à la corde (3%)
            result[k] = result[k] * 0.87;
            // Arrondir à 3 décimales
            result[k] = Math.round(result[k] * 1000) / 1000;
        });
        
        // Ajouter le poids porté comme facteur (10%)
        result.poids_porte = 0.10;
        
        // Ajouter la corde comme nouveau facteur (3%)
        result.corde = 0.03;
    ...
    calculerScoreParticipant(participant, courseContext) {
        ...
        // NOUVEAU: Calculer le bonus/malus de corde
        let cordeScore = 0;
        
        // Récupérer et traiter la valeur de corde
        const cordeValue = this.extractCordeNumber(participant.corde);
        
        if (cordeValue !== null && courseContext && courseContext.participants) {
            const totalParticipants = courseContext.participants.length;
            
            // Déterminer la catégorie de corde
            const cordeBucket = this.getCordeBucket(cordeValue, totalParticipants);
            
            // Récupérer l'ajustement de base selon la position
            let ajustementBase = this.CORDE_WEIGHTS[cordeBucket].adjustment;
            
            // Moduler l'ajustement en fonction de la distance
            const distanceBucket = this.getDistanceBucket(courseContext.distance);
            const distanceMultiplier = this.CORDE_DISTANCE_MULTIPLIERS[distanceBucket];
            
            // Calculer l'ajustement final
            const ajustementFinal = ajustementBase * distanceMultiplier;
            
            // Logs pour le débogage
            console.log(`Corde: ${cordeValue}, Total participants: ${totalParticipants}, Catégorie: ${cordeBucket}`);
            console.log(`Distance: ${courseContext.distance}m, Bucket: ${distanceBucket}, Multiplicateur: ${distanceMultiplier}`);
            console.log(`Ajustement final pour la corde: ${ajustementFinal}`);
            
            // Convertir en score (échelle 0-100)
            cordeScore = (ajustementFinal * 100);
        } else {
            console.log("Valeur de corde manquante ou invalide - facteur corde non pris en compte");
        }
        ...
        return {
            ...
            details: {
                ...
                corde: {
                    valeur: cordeValue || "NC",
                    categorie: cordeBucket || "NC",

...
const rankingLoader = {
    ...
    // NOUVEAU: Influence de la corde selon distance et taille du peloton
    CORD_POSITION_WEIGHTS: {
        "sprint": {
            "small": { "low": 0.01, "middle": 0.00, "high": -0.01 },
            "medium": { "low": 0.02, "middle": 0.00, "high": -0.02 },
            "large": { "low": 0.03, "middle": 0.00, "high": -0.03 }
        },
        "mile": {
            "small": { "low": 0.00, "middle": 0.00, "high": 0.00 },
            "medium": { "low": 0.01, "middle": 0.00, "high": -0.01 },
            "large": { "low": 0.02, "middle": 0.00, "high": -0.02 }
        },
        "middle": {
            "small": { "low": 0.00, "middle": 0.00, "high": 0.00 },
            "medium": { "low": 0.00, "middle": 0.00, "high": 0.00 },
            "large": { "low": 0.01, "middle": 0.00, "high": -0.01 }
        },
        "staying": {
            "small": { "low": 0.00, "middle": 0.00, "high": 0.00 },
            "medium": { "low": 0.00, "middle": 0.00, "high": 0.00 },
            "large": { "low": 0.00, "middle": 0.00, "high": 0.00 }
        }
    },
    ...
    // NOUVEAU: Extraire le numéro de corde à partir du format "(Corde:XX)"
    extractCordeNumber: function(cordeString) {
        if (!cordeString) return null;
        
        // Format standard "(Corde:04)" ou variantes
        ...
        const numericMatch = cordeString.match(/\d+/);
        if (numericMatch) {
    ...
    // NOUVEAU: Déterminer l'ajustement de score basé sur la corde
    getCordeFactor: function(cordeString, courseContext) {
        if (!cordeString || !courseContext) return 0;
        
        // Extraire le numéro de corde
        const cordeNumber = this.extractCordeNumber(cordeString);
        if (!cordeNumber) return 0;
        
        // Récupérer les paramètres contextuels
        const fieldSize = Array.isArray(courseContext.participants) ? courseContext.participants.length : 0;
        if (fieldSize < 3) return 0; // Trop peu de participants pour que la corde ait un impact
        
        const distanceBucket = this.getDistanceBucket(courseContext.distance);
        const fieldSizeBucket = this.getFieldSizeBucket(courseContext.participants);
        
        // Déterminer la catégorie de corde (low, middle, high)
        let cordeCategory;
        if (cordeNumber <= Math.floor(fieldSize/3)) {
            cordeCategory = "low"; // Corde intérieure (avantageuse)
        } else if (cordeNumber >= Math.ceil(fieldSize*2/3)) {
            cordeCategory = "high"; // Corde extérieure (désavantageuse)
        } else {
            cordeCategory = "middle"; // Corde moyenne (neutre)
        }
        
        // Récupérer l'impact de la corde selon le contexte
        const cordeFactor = this.CORD_POSITION_WEIGHTS[distanceBucket][fieldSizeBucket][cordeCategory] || 0;
        
        console.log(`Analyse corde: ${cordeNumber} (${cordeCategory}) dans peloton de ${fieldSize} (${fieldSizeBucket}) sur ${distanceBucket}, impact: ${cordeFactor*100}%`);
        
        return {
            factor: cordeFactor,
            category: cordeCategory,
            number: cordeNumber
        };
    },
    ...
    calculerScoreParticipant(participant, courseContext) {
        ...
        // NOUVEAU: Analyse de la corde pour son impact
        let cordeInfo = { factor: 0, category: "NC", number: null };
        
        if (participant.corde && courseContext) {
            cordeInfo = this.getCordeFactor(participant.corde, courseContext);
        }
        ...
        // NOUVEAU: Appliquer l'ajustement de corde
        if (cordeInfo.factor !== 0) {
            const scoreAvecCorde = scoreFinal * (1 + cordeInfo.factor);
            console.log(`Ajustement corde pour ${participant.cheval}: ${cordeInfo.factor > 0 ? '+' : ''}${(cordeInfo.factor*100).toFixed(1)}% (${scoreFinal.toFixed(1)} → ${scoreAvecCorde.toFixed(1)})`);
            scoreFinal = scoreAvecCorde;
        }
        ...
        return {
            ...
            details: {
                ...
                corde: {
                    numero: cordeInfo.number || "NC",
                    categorie: cordeInfo.category || "NC",
                    impact: (cordeInfo.factor * 100).toFixed(1) + "%"
                }
            }
        };
    },
